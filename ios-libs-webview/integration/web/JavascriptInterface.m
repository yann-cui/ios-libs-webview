...
- (BOOL) handleInjectedJSMethod:(NSURL *) url callBack:(NativeMethodExecHandler)handler{
    BOOL res = NO;
    if (self.interfaceProvider && [self.interfaceProvider respondsToSelector:@selector(handleInjectedJSMethod:callBack:)]) {
        res = [self.interfaceProvider handleInjectedJSMethod:url callBack:handler];
        if (!res) {
